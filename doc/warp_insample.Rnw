...
shift should be small.  Figures \ref{fig:plotoverw45} and
...
<<shift_match, cache=T, fig.cap="Maximum of shift along the measurement.">>=
matches <- function(i){
  ss <- readInd.TofH5(tofA,i)
  generated_w <- make_densewarp(ss)
  og <- optim_generator(sumSpecA, generated_w, warp0)
  sh_pos <- seq(-100, 100,length.out= 61)
  sh_data <- sapply(sh_pos, Vectorize(og))
  data.frame(ind=i/5, x=sh_pos, mv=-sh_data)
}

md <- lapply(scan_indices, matches)
...
<<searchFits, include=F, eval=T, cache=T, dependson='searchFunctions'>>=
warp_search2 <- function(ref,new){
...
wfs <- lapply(cal_indices, function(i) warp_search2(sumSpecA, readInd.TofH5(tofA,i)))
@

<<plot_fits, fig.cap="Parameter fluctuation in warp", dependson="searchFits">>=
par(mfrow=c(1,2)) 
ti <- cal_indices/5
...
In figure \ref{fig:plot_fits} we see a how the warp parameter change during the measurement.

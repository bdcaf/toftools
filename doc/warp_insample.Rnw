...
<<shift:match, cache=T, fig.cap="Maximum of shift along the measurement.">>=
local({
...
<<shift:match:local, cache=T, fig.cap="local of shift along the measurement.">>=
local({
...
<<plot:fits, fig.cap="Local warp parameter.", dependson="searchFits">>=
par(mfrow=c(1,2)) 
ti <- cal_indices/5
...
In figure \ref{fig:plot:fits} we see a how the warp parameter change during the measurement.
...
<<plot:fit1, fig.cap="Local warp parameter just shift.", dependson="searchFits">>=
ti <- cal_indices/5

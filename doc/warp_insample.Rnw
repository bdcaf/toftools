...
plot_comp <- function(refspec, otherspec = NULL, from, to,
  ...
  mass_range <- pretty(with(mcfun, c( to_mass(from), to_mass(to))), n = 10)
...
individual scans are noisy it is noticeable that the center of the first
...
spec has about half the width of individual specs.  Approximately we are
looking in the sum spec at \ac{FWHM} resolution of 1/10000 where for
individual peaks it's around 3/10000.  The correction for drift is also
around 1/10000.
...
<<searchFunctions, include = F, cache = T>>=
...
matches <- function(i, sh_pos){
  ss <- read_spec_ind.tof_h5(tof_a, i)
...
recalculate the norm, and use it although it's not exactly correct.
...
n_calibs <- 100
cal_indices <- unique(floor(seq(from = 1, to = tof_a$indexhelp$N,
                                length.out = n_calibs)))
wfs <- lapply(cal_indices,
              function(i) warp_search2(inner_reference,
                                       read_spec_ind.tof_h5(tof_a, i)))
@
...
In \fref{fig:plotfits} we see a how the warp parameter change during the
measurement.  The slope does not seem significantly different from \num{1.0}.
There doesn't appear to be a structure.  So maybe even just a plain shift will
be enough.
...
plot(sum_spec_a, type = "l", xlim = c(8e4, 8.2e4), ylim = c(0, 300))
...
cal_indices <- unique(floor(seq(from = 1, to = tof_a$indexhelp$N,
                                length.out = n_calibs)))
wfs1 <- lapply(cal_indices,
               function(i) warp_search1(inner_reference,
                                        read_spec_ind.tof_h5(tof_a, i)))
@
...
ss_pint <- ksmooth(ptime, pint, "normal", bandwidth = 16)$y
  plot(x = ptime, y = ss_pint, type = "l",
       xlab = "time [s]", ylab = "signal 59")
})
@

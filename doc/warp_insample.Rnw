...
wfs <- lapply(cal_indices, function(i) warp_search2(inner_reference, read_spec_ind.tof_h5(tof_a, i)))
@
...
n_calibs <- 50
cal_indices <- unique(floor(seq(from=1, to=tof_a$indexhelp$N, length.out=n_calibs)))
wfs1 <- lapply(cal_indices, function(i) warp_search1(inner_reference, read_spec_ind.tof_h5(tof_a, i)))
@

...
```{r}
ma59 <- make_mass_axis(mass.low=58.95, 
					   mass.high=59.15)

cal0 <- with(df, list(a=median(V1), b=median(V2)))
spec0 <- spec2mav(ss,cal0, ma59)
...
plot(ma59, allSpec, type='l')
ra <- peak_stat(allSpec, ma59)
```
#### smooth tof daq
...
plot(ma59, allSpecS, type='l')
rS <- peak_stat(allSpecS, ma59)
```


```{r}
do.call(rbind, list(r0, ra, rS))
```

...
The initial intuition was to use sparse coding - but this proved more
complex than it was worth.  May be revisited later.
...
We consider the warp function $ \tilde i = a + b \sqrt i + c i$.  For
the further discussion we want the warped signal $\tilde v$ to have the
same sum as the unwarped $v$.  This is easiest achieved when we use the
cumulative $V$ of $v$ and locally interpolate to the new indexes to a
warped cumulative $\tilde V$.  $\tilde v$ is the 2 point derivative of
$\tilde V$.
...
df <- tibble( scan=seq_len(indexhelp$N))  %>% 
  rowwise() %>%
  do(spspec(.$scan))
@

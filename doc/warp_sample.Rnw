...
tof_a <- tofH5(tof_a.h5)
tof_b <- tofH5(tof_b.h5)
sum_spec_a <- (sumSpec.TofH5(tof_a))/tof_a$indexhelp$N
sum_spec_b <- (sumSpec.TofH5(tof_b))/tof_b$indexhelp$N
...
plot_comp <- function(refspec, otherspec = NULL, from, to,
  ...
  mass_range <- pretty(with(mcfun, c( toMass(from), toMass(to))),
                      n=10)
  index_range <- with(mcfun, toIndex(mass_range))
...
plot_comp(sum_spec_a, sum_spec_b, from=11.5e4, to=13.5e4)
@
...
reference_spectrum <- matrix(sum_spec_a, nrow=1) %>%
  baseline(spectra = ., method= "rollingBall",
           wm=200, ws=200) %>%
  slot("corrected") %>% t %>%
  pmax(0)

plot_comp(sum_spec_a, reference_spectrum, from=12.0e4, to=13.0e4,
          ylim=c(0, 2))
@

<<wssat, cache=F, depends=c("wsbaseline"), fig.cap="saturated peaks">>=
par(mfrow=c(1, 2))
plot_comp(reference_spectrum, from=7.99e4, to=8.02e4)
plot_comp(reference_spectrum, from=9.96e4, to=9.98e4)
@
...
finding the globaly best warping demanding.  Comparing with
...
peaks are not regulary spaced in time axis these secondary peaks have
much poorer match.
...
shifts <- seq(from=-6000, to=3000, length.out=99)
val <- sapply(shifts, og)
@

<<wscalcoptshift, include=F, cache=T>>=
best_shift <- local({
  ...
  nloptr( x0 = first_shift$solution,
         ...
         lb=-3000,
         ub = 3000,
         opts = list(algorithm="NLOPT_LN_BOBYQA", xtol_abs=0.01)
         )
})
@
...
abline(v=best_shift$solution, lty=2)
text(x=best_shift$solution, y=0.5,
     sprintf("%3.1f", best_shift$solution), pos=4)
@
...
make_match_surf <- function(xs, ys) {
  ...
  vals <- outer(xsc, ysc, FUN=wif1 )
  list(xs=xsc, ys=ysc, vals=vals)
}

@


<<plotfunc, cache=T, include=F>>=
plot_match <- function(match_surf) {
  with(match_surf, {
         ...
         with(best_shift, {
                nc <- trans3d(x=solution[[1]], y=solution[[2]], z=-objective, pm)
                points(nc, col="red")
...
large_scale <- make_match_surf(
  xs = seq(from=-800, to=-200, length.out=31),
  ys = seq(from=0.98, to=1.02, length.out=31)
)
@

<<wsshiftplot2, cache=T, dependson=c("wscalcshift2",'wshift_opt2', "plotfunc"), fig.cap="distance matrix to reference measurement on shifted and compressed spectrum", dependson="wscalcshift2">>=
plot_match(large_scale)
@

<<wscalcshift2local, include=F, cache=T>>=
medium_scale <- make_match_surf( xs = seq(from=-700, to=-300, length.out=31),
                                 ys = seq(from=0.995, to=1.005, length.out=31))
@

<<wsshiftplot2local, cache=T, dependson=c("wscalcshift2local",'wshift_opt2', "plotfunc"), fig.cap="local distance matrix to reference measurement on shifted and compressed spectrum", dependson="wscalcshift2">>=
plot_match(medium_scale)
@

<<wscalcshift2locallocal, include=F, cache=T>>=
small_scale <- make_match_surf(xs = seq(from=-550, to=-450, length.out=31),
                               ys=seq(from=0.999, to=1.001, length.out=31))
@
<<wsshiftplot2locallocal, cache=T, dependson=c("wscalcshift2locallocal",'wshift_opt2', "plotfunc"), fig.cap="direct environment around maximum of distance matrix to reference measurement on shifted and compressed spectrum", dependson="wscalcshift2">>=
plot_match(small_scale)
@


\subsubsection{Algortithm Candidate}
...
<<wscalcshift1f, include=F, cache=T>>=
vander <- function(xs, nmax)
  outer(xs, seq(from=0, to=nmax), function(x, i) x^i)
...
<<wsqualdim, dependson=c('wscalcshift1f',"calc_orders"), cache=T, fig.cap="improvent in distance by increasing warp polynomial.  Lower is a better match.">>=
local({
qs <- -sapply(warp_orders, function(x) x$objective)
lik <- qs/ (1-qs)
ll <- log(lik)

plot(seq_along(qs), log(1-qs), xlab="poly order", ylab="log cosine distance", type="p")
})
@

<<, dependson=c("calc_orders")>>=
local({
...
rv0 <- res0(orig)
rv <- res(orig)
mv_orig <- mcfun$toMass(orig)
mv_new <- mcfun$toMass(rv)
mv_new0 <- mcfun$toMass(rv0)
md <- mv_new-mv_orig
md0 <- mv_new0-mv_orig
plot(mv_orig, md, type="l")
lines(mv_orig, md0, type="l", col="green")

plot(mv_orig, mv_new-mv_new0, type="l")
plot(mv_orig, (mv_new-mv_new0)/mv_orig, type="l")
plot(mv_orig, ssa, type="l")
@

...
Various warp functions are available.  As TOF spectrometry has
...
xs_local2<- sort(c(bestbest$par[[1]],seq(from=-600, to=-400, length.out=31)))
ys_local2<- sort(c(bestbest$par[[2]],seq(from=0.998, to=1.002, length.out=31)))
...
actually form a connected smooth \textit{ridge}. Still finding the
maximum is sensitive to the start location.  Only few optimization
algorithm deal with such a situation sufficiently.  Here a genetic
algorithm is applied.  Ultimately this search is done in two steps.
First using a genetic algorithm to locate a correlation > 0.5 and in a
second step conjugate gradient optimization to locate the maximum.  This
is significantly faster than using the genetic algorithm exclusively as
it only slowly converges to the maximum. 
...
@
<<wsshiftplot2locallocal, cache=T, dependson=c("wscalcshift2locallocal",'wshift_opt2'),fig.cap="local distance matrix to reference measurement on shifted and compressed spectrum", dependson="wscalcshift2">>=
par(mar=c(1,0,0,0))

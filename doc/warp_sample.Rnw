...
generated_w <- make_densewarp(v, 1, length(v))
warp_fun <- warp2(c(est.shi$optim$bestmem, 0.992, 0.0000001))
starts <- 1
ends <- length(ssb)
v <- ssb
check <- T
warp_res <- warper(warp_fun)

og_fin <- optim_generator(ssa, warp2)
og_fin2 <- function(...) -og_fin(...)

bestbest <- optim(startv, 
				  og_fin2, 
				  gr=NULL, 
				  method='Nelder-Mead',
				  ssb)

...
finding the globaly best warping demanding.  Comparing with figure
\ref{fig:wscleansum} we see that in this mass range there are around
...
<<ws:calc:shifts,include=F, cache=T>>=
shifts <- seq(from=-6000, to=3000, length.out=99)
val <- sapply(shifts, og)
@

<<ws:calc:opt:shift, include=F, cache=T>>=
best_shift <- local({
...
text(x=best_shift$solution, y=0.5, sprintf("%3.1f",best_shift$solution), pos=4)
@
...
As now polynoms of higher degree can be fitted we see the corrections
are small compared to the shift.  It may be sufficient to correct just
for a linear shift.
...
We see in fig. \ref{fig:ws:qual:dim} that with increasing polynomial order the cosine distance is initially not improved. 

<<ws:qual:dim, dependson=c('wscalcshift1f',"calc_orders"), cache=T, fig.cap="improvent in distance by increasing warp polynomial.">>=
local({

...
optim_generator <- function(orientSpec, wf){
  ...
  function(a, generated_w){
	warp_fun <- wf(a)
	warped <- generated_w(warp_fun)
...
og2 <- function(...) -og(...)
generated_w <- make_densewarp(ssb, 1, length(ssb))
#og(1,ssb)
shifts <- seq(from=-3000, to=2000, length.out=99)
val <- sapply(shifts, function(x) og(x,generated_w))
...
og2 <- function(...) -og(...)
wif <- Vectorize(function(a,b) og(c(a,b), generated_w))
#og(1,ssb)
@

...
plotComp <- function(refspec, otherspec, from, to, ylim=c(0,1e2)){
  plot(refspec, type='l', 
	   xlim=c(from, to),
	   ylim=ylim,
	   xlab='index',
  ...
  lines(otherspec, type='l', col='red')

  leftMass <- with(mcfun, ceiling(toMass(from)))
  rightMass <- with(mcfun, floor(toMass(to)))
  massRange <- seq(from = leftMass, to = rightMass)
  indRange <- with(mcfun, toIndex(massRange))
  axis(1,indRange, labels=massRange,line=1,col="red",col.ticks="red",col.axis="red")
}
@
...
plotComp(sumSpecA, refSpec, from=12.0e4, to=13.0e4, ylim=c(0,3))
@

...
abline(v=bestbest$par, lty=2)
text(x=bestbest$par, y=0.5, sprintf("%3.1f",bestbest$par), pos=4)
@

<<,include=F>>=
og <- optim_generator(ssa, generated_w, warp1)
og2 <- function(...) -og(...)
wif <- Vectorize(function(a,b) og(c(a,b)))
#og(1,ssb)
@
...
og <- optim_generator(reference, generated_w, warp0)
...
est0 <- optim(est0_pre$optim$bestmem, og,
				  gr=NULL, 
				  lower=-3000, 
				  upper=3000,
				  method='Brent')
...
ni <- 2
ni_fit <- function(ni, preval){
  starta <- if (length(preval)>1) c(preval,0) else c(preval,1)
  polywarp <- make_polywarp(ni)
  og_fin <- optim_generator(reference, generated_w, polywarp)
  bestbest <- optim(starta, og_fin, method="Nelder-Mead")
}
...
We see in fig. \ref{fig:ws:qual:dim} that with increasing polynomial the
improvement after the linear term is disappearing.  It seems reasonable
that the time shift is compromised of only a constant and linear term.
Even at logarithmic scale improvement beyond the linear term is
inconceivable.  And probably can be argued away considering the extra
degree of freedom.

<<ws:qual:dim, dependson='wscalcshift1f', cache=T, fig.cap="improvent in distance by increasing warp polynomial.">>=
qs <- -sapply(bb, function(x) x$value)
lik <- qs/(1-qs)
ll <- log(lik)
...
warp_searcher <- function(ref,new,warper){
  ...
  og <- optim_generator(ref, make_densewarp(new), warp0)
  og2 <-   function(...) -og(...)
  est.shi <- DEoptim(og2, 
					 lower=-3000,
					 upper = 3000,
  ...
  og_fin <- optim_generator(ssa, generated_w, warp2)
  og_fin2 <- function(...) -og_fin(...)

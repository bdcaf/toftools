...
ys <- seq(from=0.9, to=1.1, length.out=17)
...
<<wsshiftplot2, cache=T, dependson="wscalcshift2",fig.cap="distance matrix to reference measurement on shifted and compressed spectrum", dependson="wscalcshift2">>=
image(xs,ys,log(vals), xlab='shift', ylab='scale')
with(bestbest, {
	   points(x=par[[1]], y=par[[2]])
	   text(x=par[[1]], y=par[[2]], sprintf("(%3.1f, %1.3f)",par[[1]], par[[2]]), pos=2) 
	   			  })
@

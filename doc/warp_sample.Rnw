...
prep_spec <- function(sspec, atof, wid=30){
  ...
  workspec <- zapsmall(log1p(workspec), digits=2)
  workspec <- ksmooth(seq_along(workspec), workspec, kernel='normal', bandwidth=16)$y
}
...
og <- optim_generator(ssa, generated_w, warp0)
...
<<,include=F>>=
og1 <- optim_generator(ssa, generated_w, warp1)
#og(1,ssb)
@
...
for (i in 1:5){
  nr <- ni_fit(i, pre)
  out <- c(warp_orders, list(nr))
  pre <- nr$solution
}
@

We see in fig. \ref{fig:ws:qual:dim} that with increasing polynomial order the
...
plot(seq_along(qs)-1, log(1-qs), xlab='poly coef', ylab='log cosine distance', type='p')
})
@

...
ys <- seq(from=0.98, to=1.02, length.out=41)
...
bestbest <- optim(est.shi$optim$bestmem, 
				  ...
				  ssb)
@
In figure \ref{fig:wsshiftplot2} the two dimensional case is shown.  We
see that there is a comb of agreement with a local maximum.  This makes
finding the best location sensitive to the start location.  Only few
...
maximum.  This is significantly faster than using the genetic algorithm

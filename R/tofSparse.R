...
semisparse_spec <- function(full.wave,  lower=0L,  minlen=1L,  max_gap = 30L){
  ...
  table_spec <- tab[,  .(starts=min(id),
                  ends=max(id),
                  v=list(Reduce(c,  v))),
               by=id_block]
  table_spec[,  len := ends-starts+1]
  table_spec[ends>starts]
  return( table_spec[len>=minlen])
}
...
#" spectrum.
#" @param spec semisparse spectrum
#" @param reference_spec reference spectrum
#" @param refEnergy energy of the reference spectrum may be
#" pre-calculated
#" @return cosine distance of spectra
#"
cor_semisparse_full <- function(spec,  reference_spec){
  spec[,  energy := Vectorize(function(x) sum(x ^ 2))(v)]
  spec[,  sp := Vectorize( function(st, en, v) spfun(st, en, v,  reference_spec))(starts,  ends, v)]
  agg <- spec[,  .(total_energy=sum(energy),  total_sp=sum(sp)),  by=NULL]
  with(agg,  total_sp/sqrt(total_energy))
}

cor_full_full <- function(warp_result,  reference_spec){
  if (is.na(warp_result$starts)) return(0)
  effective_range <- with(warp_result,  starts:ends)
  effective_range[effective_range<1] <- NA
  effective_range[effective_range>length(reference_spec)] <- NA
  sp <- with(warp_result,  sum(reference_spec[effective_range] * v,  na.rm=T))
   total_energy <- with(warp_result,  sum(v^2, na.rm=T))
